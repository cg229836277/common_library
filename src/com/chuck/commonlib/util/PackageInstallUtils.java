...
import com.chuck.commonlib.util.ShellUtils.CommandResult;
...
public class PackageInstallUtils {
     ...
     * 
     * @param context
     * @param filePath apk本地地址
     * @return
     */
    public static final int install(Context context, String filePath) {
    	boolean isRoot = ShellUtils.checkRootPermission();
        if (PackageInstallUtils.isSystemApplication(context) || isRoot) {
     ...
     * @param context
     * @param filePath file path of package
     * @return {@link PackageInstallUtils#INSTALL_SUCCEEDED} means install success, other means failed. details see
     *         {@link PackageInstallUtils}.INSTALL_FAILED_*. same to {@link PackageManager}.INSTALL_*
     ...
     * 
     * @author admin
     * @date 2015-5-9 下午4:07:14
     * @param context
     * @param filePath 需要安装的apk的本地地址
     * @param pmParams 
     * @return
     */
    public static int installSilent(Context context, String filePath, String pmParams) {
        ...
        //需要安装的apk文件，获取文件名
        String targetFileName = filePath.replace(" ", "\\ ").substring(filePath.lastIndexOf(File.separator) + 1, filePath.length());
        //将需要安装的apk复制到/system/app下
        String destFilePath = " /system/app/";
        //在ROOT状态下挂载，使得System可以读写
        StringBuilder mountCommand = new StringBuilder().append("LD_LIBRARY_PATH=/vendor/lib:/system/lib mount -o remount,rw -t yaffs2 /dev/block/mtdblock3 /system");
        CommandResult getMountResult = ShellUtils.execCommand(mountCommand.toString(),true, false);
        
        StringBuilder copyCommand = new StringBuilder().append("LD_LIBRARY_PATH=/vendor/lib:/system/lib cp ").append(filePath.replace(" ", "\\ ")).append(destFilePath);
        CommandResult getCopyResult = ShellUtils.execCommand(copyCommand.toString(),true, false);
        
        StringBuilder executeCommand = new StringBuilder().append("LD_LIBRARY_PATH=/vendor/lib:/system/lib chmod 755 ").append(destFilePath + targetFileName);     
        CommandResult getExecuteResult = ShellUtils.execCommand(executeCommand.toString(),true, true);
        
        StringBuilder command = new StringBuilder().append("LD_LIBRARY_PATH=/vendor/lib:/system/lib pm install ").append(pmParams == null ? "" : pmParams).append(" ").append(destFilePath + targetFileName);
        boolean isNeedRoot = !isSystemApplication(context);
        CommandResult commandResult = ShellUtils.execCommand(command.toString(),isNeedRoot, true);
        
        StringBuilder unmountCommand = new StringBuilder().append("LD_LIBRARY_PATH=/vendor/lib:/system/lib mount -o remount,ro -t yaffs2 /dev/block/mtdblock3 /system");
        CommandResult getUnMountResult = ShellUtils.execCommand(unmountCommand.toString(),true, false);
    ...
    public static final int uninstall(Context context, String packageName) {
        if (PackageInstallUtils.isSystemApplication(context) || ShellUtils.checkRootPermission()) {
    ...
    public static Boolean isTopActivity(Context context, String packageName) {
        ...
        if (CollectionUtil.isListNull(tasksInfo)) {
            return null;
        }
        try {
